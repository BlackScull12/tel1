...
async function loadUsers() {
  ...
  snapshot.forEach((docu) => {
    ...
    const name = document.createElement("span"); name.innerText = user.name;
    const status = document.createElement("span"); status.innerText = user.online ? "🟢" : "⚫";
    const unread = document.createElement("span"); unread.className = "unreadBadge"; unread.id = "unread_" + docu.id;
    div.appendChild(img); div.appendChild(name); div.appendChild(status); div.appendChild(unread);
  ...
  onSnapshot(messagesQuery, async (snapshot) => {
    ...
    snapshot.forEach(async (docu) => {
      ...
      if (msg.sender === currentUser.uid) {
        const tick = document.createElement("span"); tick.className = "tick"; tick.innerText = msg.seen ? "✔✔" : "✔"; bubble.appendChild(tick);
      } else if (!msg.seen) {
        await updateDoc(doc(db, "chats", chatID, "messages", docu.id), { seen: true });
      }
      const time = document.createElement("div"); time.className = "time"; const date = new Date(msg.time); time.innerText = date.getHours() + ":" + String(date.getMinutes()).padStart(2, "0"); bubble.appendChild(time);
...
async function sendMessage() {
  ...
  await addDoc(collection(db, "chats", chatID, "messages"), { text: input.value, sender: currentUser.uid, time: Date.now(), seen: false });
  input.value = "";
}
...
async function clearChat() {
  ...
  snapshot.forEach(async (docu) => { await deleteDoc(doc(db, "chats", chatID, "messages", docu.id)); });
}
...
function listenUnread(uid) {
  ...
  onSnapshot(messagesQuery, async (snapshot) => {
    ...
    let count = 0;
    snapshot.forEach((docu) => { const msg = docu.data(); if (msg.sender === uid && msg.time > lastRead) count++; });
